...
function ACO = jAntColonyOptimization(fun,feat,label,opts)
% Parameters
...
while t <= max_Iter
  ...
  for i = 1:N
    % Fitness
    [fit(i), Acc(i), AUC(i), AUPRC(i), fscore(i)] = fun(feat,label,X_bin(i,:),num);
    % Global update
    if fit(i) < fitG
      Xgb  = X(i,:);
      fitG = fit(i);
...
ACO.f  = feat;
ACO.l  = label;
ACO.training_auc = 1 - (fitG - 0.1 * (ACO.nf / dim)) / 0.9;
end
